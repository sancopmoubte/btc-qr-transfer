...
- 二维码使用 640×640 的真实画布、Q 级纠错和完整白色静区；接收失败时可以重新显示或重新下载同一张二维码，不会创建新 nonce，也不会再次扣款。
- 生成流程采用事务顺序：二维码在临时容器中成功生成并通过尺寸检查后，才替换正式二维码、扣除余额和保存 `pendingVoucher`；生成失败时余额和旧二维码均保留。
- 二维码生成、识别和压缩脚本已经放在仓库的 `vendor/` 目录，不依赖 CDN、不依赖构建工具、不依赖 GitHub Actions。
- 接收端使用运行时 `processingNonces` 锁和统一的 `receiveVoucherOnce` 入口，自动识别、手动验证和重复点击共享同一幂等流程；同一凭证只能入账一次，刷新后也会通过 `usedNonces` 拒绝重复接收。
